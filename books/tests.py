...
class BookTestCase(TestCase):
    ...
    def test_borrow_return(self):
        response = self.client.post('/book/2/borrow', headers=self.headers, data={'id': '2', 'qrcode': settings.QRCODE}, content_type='application/json')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(Book.objects.get(id='2').availability, 'rented')

        response = self.client.post('/book/2/return', headers=self.headers, data={'id': '2', 'qrcode': settings.QRCODE}, content_type='application/json')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(Book.objects.get(id='2').availability, 'available')


    def test_borrowing_book_with_invalid_qr(self):
        response = self.client.post('/book/2/borrow', headers=self.headers, data={'id': '2', 'qrcode': settings.QRCODE + 'a'}, content_type='application/json')
    ...
    def test_borrowing_unavailble_book(self):
        response = self.client.post('/book/1/borrow', headers=self.headers, data={'id': '1', 'qrcode': settings.QRCODE}, content_type='application/json')
        self.assertEqual(response.json()['error'], 'ERR_UNABLE_TO_BORROW')


    def test_borrowing_invalid_book(self):
        response = self.client.post('/book/10/borrow', headers=self.headers, data={'id': '10', 'qrcode': settings.QRCODE}, content_type='application/json')
    ...
    def test_getting_borrowed_books1(self):
        self.client.post('/book/2/borrow', headers=self.headers, data={'id': '2', 'qrcode': settings.QRCODE}, content_type='application/json')
        ...
        self.client.post('/book/2/return', headers=self.headers, data={'id': '2', 'qrcode': settings.QRCODE}, content_type='application/json')
